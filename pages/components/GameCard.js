...
const GameCard = ({ game, onRemove }) => {
  ...
  const changeRating = async (newRating) => {
    ...
    if (newRating >= 4) {
      const response = await axios.get(`/api/recommendations?slug=${game.slug}`);
  ...
  return (
    <div className="bg-white p-6 mb-6 rounded shadow">
      ...
      {similarGames.length > 0 && (
        <div>
          <h4 className="text-lg font-semibold mt-6 mb-4">Similar games you may like:</h4>
          <div className="grid grid-cols-2 gap-4">
            {similarGames.map((similarGame) => (
              <div key={similarGame.id} className="bg-gray-100 p-4 rounded shadow">
                <img
